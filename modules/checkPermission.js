...
module.exports = {
    Member: (command, message) => {
        ...
        return message.channel.send(`${emojis.deny} **|** ${message.author}, você não pode executar esse comandos, pois você precisa das permissões de ${permissions.join(", ")}.`).then(msg => { msg.delete({ timeout: 15000 }) });
    },
    Client: (command, message) => {
        ...
        return message.channel.send(`${emojis.deny} **|** ${message.author}, eu não posso executar esse comandos, pois eu preciso das permissões de ${permissions.join(", ")} nesse servidor.`).then(msg => { msg.delete({ timeout: 15000 }) });
    }
};
